...
export const edit_fabric_master_by_id = async(req, res)=>{
    ...
    try{
        ...
        if(!response) return res.status(405).json(new apiResponse(405, responseMessage?.updateDataError("fabric"),{},{}))
...
export const delete_fabric_master_by_id = async(req, res)=>{
    ...
    try{
        ...
        if(!response) return res.status(400).json(new apiResponse(400, responseMessage?.getDataNotFound("fabric"),{},{}))
...
export const get_by_id_fabric_master = async(req, res)=>{
    ...
    try{
        ...
        if(!response) return res.status(400).json(new apiResponse(400, responseMessage?.getDataNotFound("fabric"),{},{}))
        return res.status(200).json(new apiResponse(200, responseMessage?.getDataSuccess("fabric"),response,{}))
    }catch(error){
        console.log(error);
        return res.status(500).json(new apiResponse(500, responseMessage?.internalServerError, {}, error))
    }
